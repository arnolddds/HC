...
class HeatConduction2D:
    def __init__(self, L=0.5, H=0.5, Nx=50, Ny=50, t_end=60, dt=0.1, Th=80, Tc=30, T0=5):
        """
        Инициализация решателя задачи теплопроводности в 2D.
    ...
    def solve(self):
        """Решение задачи теплопроводности с использованием явного метода конечных разностей."""
        ...
        for step in range(num_steps):
            ...
            # Обновление поля температуры (явная схема)
            for i in range(1, self.Ny - 1):
                for j in range(1, self.Nx - 1):
                    T_new[i, j] = (
                        self.T[i, j]
                        + r_x * (self.T[i, j + 1] - 2 * self.T[i, j] + self.T[i, j - 1])
                        + r_y * (self.T[i + 1, j] - 2 * self.T[i, j] + self.T[i - 1, j])
                    )
...
t_end, dt = 60, 0.01  # Время симуляции и временной шаг

# Создание и решение задачи
solver = HeatConduction2D(L, H, Nx, Ny, t_end, dt, Th, Tc, T0)
solver.solve()
solver.plot()
